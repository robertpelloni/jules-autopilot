# Borg Integration Contract

This document outlines how the **Jules Autopilot** acts as a sub-component within the larger **Borg** ecosystem.

## Architecture Role
Jules Autopilot is the **Cloud Session Orchestrator**. 
While Borg manages local tasks, file manipulation, and high-level reasoning across multiple repositories, Jules Autopilot is solely responsible for spinning up, monitoring, and steering parallel cloud coding instances (like Google Jules) via its REST API.
...
Borg can simply inject tasks into the queue or read the SQLite database (`prisma/dev.db`) directly to see event histories.

### 3. Shared Intelligence (`@jules/shared`)
Borg agents should utilize the `@jules/shared/orchestration` package to run Multi-Agent Debates (`debate.ts`) and evaluate risk (`supervisor.ts`) before executing destructive local actions. The models and logic have been normalized so both Borg and Jules use the exact same evaluation criteria.

## Handoff State
The `.borg` folder tracks active assimilation state. When launching Borg in this repository, it should read `HANDOFF.md` to understand the current capabilities of the fleet.