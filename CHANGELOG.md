...
- **Archive empty state:** Button now falls back to archived sessions when none are unarchived, so it always creates new sessions
...
- **Repeated GitHub clone error detection:** Scans session activities for "github + clone + error/fail/timeout" patterns and logs hourly aggregate
