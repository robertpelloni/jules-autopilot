...
import { Sparkles, Trash2, Loader2, Download, Users, Plus } from 'lucide-react';
  ...
  return (
    <ScrollArea className="flex-1 px-6 py-4 h-full">
      <div className="space-y-6 pb-8">
        ...
        {/* Shadow Pilot Settings */}
        <div className="flex flex-col gap-4 border border-rose-500/20 p-4 rounded-lg bg-rose-500/5">
          <div className="flex items-center justify-between">
            <Label htmlFor="shadow-pilot" className="flex flex-col gap-1">
              <span className="font-semibold text-sm flex items-center gap-2 text-rose-400">
                <Sparkles className="h-4 w-4" />
                Shadow Pilot Mode
              </span>
              <span className="font-normal text-xs text-white/40">
                Silent Guardian: Analyze local git diffs for catastrophic errors and secrets
              </span>
            </Label>
            <Switch
              id="shadow-pilot"
              checked={config.shadowPilotEnabled}
              onCheckedChange={(c) => handleConfigChange({ ...config, shadowPilotEnabled: c })}
            />
          </div>
        </div>
