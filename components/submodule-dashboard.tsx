...
import { GitBranch, Clock, FolderGit2, Hash, BookOpen, Info } from "lucide-react";
...
export function SubmoduleDashboard() {
  const { data, error, isLoading } = useSWR<{ submodules: Submodule[], generatedAt: string }>('/api/system/submodules', fetcher, {
    refreshInterval: 60000 // refresh every minute
  });

  if (error) {
    return <div className="p-6 text-red-500">Failed to load submodule data</div>;
  }
    ...
    <div className="space-y-6 p-6 max-w-7xl mx-auto">
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-3">
          ...
          <Badge className="bg-blue-500/10 text-blue-400 border-blue-500/20">v0.9.2</Badge>
          {isLoading && <Badge className="bg-zinc-800 text-zinc-400 border-zinc-700 animate-pulse">Syncing...</Badge>}
                ...
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <Badge variant="outline" className={`text-[10px] uppercase tracking-wider font-mono ${getStatusColor(submodule.status)}`}>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between py-1 border-t border-white/5">
                      ...
                      <span className="text-white/80">
                        {submodule.lastCommitDate && !isNaN(new Date(submodule.lastCommitDate).getTime()) ? formatDistanceToNow(new Date(submodule.lastCommitDate), { addSuffix: true }) : 'Unknown'}
                      </span>
                    </div>
