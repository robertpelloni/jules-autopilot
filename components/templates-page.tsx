    ...
    <div className="flex-1 overflow-y-auto p-8 bg-black">
      <div className="max-w-6xl mx-auto space-y-8">
        <div className="flex items-center justify-between border-b border-white/[0.08] pb-6">
          <div>
            <h2 className="text-sm font-bold text-white tracking-widest uppercase">Session Templates</h2>
            <p className="text-[11px] text-white/40 mt-1 uppercase tracking-wide font-mono">Manage and use your reusable prompt templates</p>
          </div>
          <Button 
        ...
        {templates.length === 0 ? (
          <div className="text-center py-20 border border-dashed border-white/10 rounded-lg bg-white/5">
            ...
            <h3 className="text-sm font-bold text-white mb-2 uppercase tracking-wide">No templates yet</h3>
            ...
            <Button onClick={() => setIsCreateOpen(true)} variant="outline" className="h-8 text-[10px] uppercase tracking-widest font-mono">
              Create Template
            </Button>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {templates.map((template) => (
              <Card key={template.id} className="bg-zinc-950 border border-white/10 hover:border-purple-500/30 transition-all group flex flex-col rounded-sm">
                <CardHeader className="flex-1 p-4 pb-2">
                  <div className="flex justify-between items-start gap-2 mb-2">
                    <CardTitle className="text-xs font-bold text-white uppercase tracking-wide leading-tight">{template.name}</CardTitle>
