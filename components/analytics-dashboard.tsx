...
export function AnalyticsDashboard() {
  ...
  const stats = useMemo(() => {
    const { sessions: currentSessions, activities: currentActivities } =
      filteredData;
    ...
    const completedSessions = currentSessions.filter(
      (s) => s.status === "completed",
    ).length;
    const failedSessions = currentSessions.filter(
      (s) => s.status === "failed",
    ).length;
    ...
    // Activity breakdown
    const activityTypes = currentActivities.reduce(
      (acc, curr) => {
    ...
    // Repository usage
    const repoCounts = currentSessions.reduce(
      (acc, curr) => {
        ...
        <div className="flex items-center justify-between pb-3 border-b">
          ...
          <div className="flex items-center gap-2">
            <Select value={dateRange} onValueChange={setDateRange}>
              <SelectTrigger
                className="w-[140px] h-8 text-xs text-foreground"
            ...
            <Button
              ...
              className="h-8 w-8 hover:bg-primary/10 hover:border-primary/50 transition-colors"
            >
              <RefreshCw
