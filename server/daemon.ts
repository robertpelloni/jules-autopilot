...
import { superviseSession, getFreeModels } from './supervisor';
import { getSupervisorAPIKey } from './llm';
...
export class Daemon {
    ...
    async start() {
        ...
        // Pre-fetch free models on start
        const settings = await prisma.keeperSettings.findUnique({ where: { id: 'default' } }).catch(() => null);
        if (settings) {
            const apiKey = getSupervisorAPIKey('openrouter', settings.supervisorApiKey);
            if (apiKey) {
                getFreeModels(apiKey).catch(e => console.error('[Daemon] Failed to pre-fetch free models:', e));
            }
        }
    ...
    private async autopilotLoop() {
        ...
        while (this.isRunning) {
            ...
            const interval = (s?.checkIntervalSeconds || 600) * 1000;
