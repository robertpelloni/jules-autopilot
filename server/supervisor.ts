    ...
    try {
        ...
        const models = (data.data || [])
            .filter((m: any) => {
                ...
                // Filter for chat/instruct models only (avoid OCR, vision, etc.)
                const id = m.id.toLowerCase();
                return id.includes('chat') || id.includes('instruct') || id.includes('it') || id.includes('llama') || id.includes('mistral') || id.includes('gemma') || id.includes('qwen');
            })
            .map((m: any) => m.id)
            .sort();
...
export async function superviseSession(
    ...
    // Strictly use OpenRouter
    const provider = 'openrouter';
    const apiKey = getSupervisorAPIKey(provider, dbSettings?.supervisorApiKey);
    
    if (!apiKey) throw new Error(`No OpenRouter API key found in environment or settings`);
    
    // Build model rotation list from OpenRouter free models
    const freeModels = await getFreeModels(apiKey);
    
    // Cycle through models: start from nextModelIndex and wrap around
    const modelsToTry = [];
    if (freeModels.length > 0) {
        for (let i = 0; i < freeModels.length; i++) {
            modelsToTry.push(freeModels[(nextModelIndex + i) % freeModels.length]);
        }
        // Increment global index for next session
        nextModelIndex = (nextModelIndex + 1) % freeModels.length;
    } else {
        modelsToTry.push(dbSettings?.supervisorModel || 'google/gemma-3-27b-it:free');
    }
