...
api.post('/rag/query', async (c) => {
    try {
        ...
        const settings = await prisma.keeperSettings.findUnique({ where: { id: 'default' } }).catch(() => null);
        const apiKey = process.env.OPENROUTER_API_KEY || settings?.supervisorApiKey;

        if (!apiKey || apiKey === 'placeholder') {
            return c.json({ error: 'OpenRouter API key is required for RAG' }, 401);
        }
...
api.post('/settings/keeper', async (c) => {
    try {
        ...
        const data = {
            ...
            supervisorProvider: String(body.supervisorProvider || 'openrouter'),
            supervisorApiKey: body.supervisorApiKey || null,
            supervisorModel: String(body.supervisorModel || 'google/gemma-3-27b-it:free'),
